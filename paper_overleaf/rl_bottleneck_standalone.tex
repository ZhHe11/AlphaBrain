...
\begin{figure}[t]
    ...
    \caption{\textbf{Single-GPU frozen-VLA RL vs.\ full-VLA fine-tuning (compute-matched, ours).} Offline 50-ep all-task success rate per checkpoint. RLT$_a$+PPO (teal) learns from a cold $0$ and converges above the small-scale full-VLA baselines (dashed), which begin from the measured $0.704$ SFT base. This is the efficiency/stability claim of RQ-B---\emph{not} a comparison against large-scale full-VLA RL (RLinf-VLA, $0.988$; see \cref{tab:rlt_cost}), which is not shown here.}
    \label{fig:rlt_vs_vla}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.82\linewidth]{figures/rlt_large_scale_comparison.png}
    \caption{\textbf{Large-scale full-VLA reference.} RLT$+$PPO reaches $0.953$ on a single GPU while training only ${\sim}0.8$M parameters, exceeding our faithfully scaled FSDP-$6$ full-VLA PPO baseline ($0.922$) and reaching ${\sim}96\%$ of RLinf-VLA's reported $0.988$ with far fewer trainable parameters and GPUs. This is the large-scale caveat and efficiency frontier behind \cref{tab:rlt_cost}.}
    \label{fig:rlt_large_scale}
\end{figure}
