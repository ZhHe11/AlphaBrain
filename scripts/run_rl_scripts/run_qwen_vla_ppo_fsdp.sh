#!/bin/bash
# Vanilla VLA + PPO (full finetune) — FSDP via torchrun, RLinf-aligned. [Phase 2 final]
#
# Each rank = 1 GPU: VLM backbone FSDP-sharded (FULL_SHARD across all ranks);
# action_model + value_head replicated per rank with manual NCCL all-reduce.
# Rollout: step-lock batched collector over a per-rank persistent env pool.
# Launcher: torchrun (no Ray).
#
# Defaults are RLinf-PPO numbers (see RLINF_VLA_PPO_GRPO.md):
#   G=128, micro_batch=80, lr_vla=1e-4, lr_critic=3e-3
#
# Usage:
#   bash scripts/run_rl_scripts/run_qwen_vla_ppo_fsdp.sh
#   GPUS=0,1,2,3,4,5,6,7 MAX_ITER=300 bash scripts/run_rl_scripts/run_qwen_vla_ppo_fsdp.sh
#
# Env:
#   GPUS            comma-separated GPU ids (default 0,1,2,3,4,5,6,7)
#   G               TOTAL episodes/iter (default 128, RLinf-PPO)
...
#   MICRO_BATCH     VLA re-forward batch in PPO update (default 80, RLinf-PPO)
#   LR_VLA          VLA full-FT LR (default 1e-4, RLinf-PPO)
#   LR_CRITIC       value-head LR (default 3e-3, RLinf-PPO)
#   MAX_ITER        total iterations (default 4 — verification; pass 300 for real)
...
RUN_TAG="vla_ppo_fsdp_qwen_${RUN_SUFFIX}"
OUTPUT_DIR="results/rlt_training/${RUN_TAG}_${TIMESTAMP}/vla_ppo"
mkdir -p "${OUTPUT_DIR}"
TRAIN_LOG="${OUTPUT_DIR}/train.log"

echo "============================================================"
echo " Vanilla VLA + PPO (FULL FT) — FSDP torchrun [RLinf-aligned]"
...
echo "   lr_vla:        ${LR_VLA}   lr_critic: ${LR_CRITIC}"
