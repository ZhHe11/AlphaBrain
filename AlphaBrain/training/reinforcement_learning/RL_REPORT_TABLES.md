...
| 算法 | 类型 | critic | 终态 SR(终值,task0/1/3) | 样本效率 (env-steps→80%) | 稳定性 (seed 方差) | 代码状态 |
|:-----|:-----|:-------|:---------------------------|:---------------------------|:-------------------|:---------|
| TD3 | off-policy AC | 双 Q + 目标平滑 | 0.98 / 0.92 / 0.64 | —(见下注,不报)| — | ✅ 可跑 |
...
> ⚠️ **样本效率(env-step)列不报,fig6c 已删(0610)**。原因:env-step 轴对 off/on-policy **不公平**。实测各 run 采集率:**TD3 ~48.8k env-step/iter,PPO ~8.5k,GRPO ~10.2k**(源各 run `metrics.json` `total_env_steps`)——TD3 off-policy 每 iter 故意多采 ~5.7× 喂 300k replay buffer。所以"TD3 到 80% 要 ~20× env-step"是**采集率假象**,不是数据效率低:off-policy 的效率在**数据复用**(每 transition 反复用、梯度更新数 >> 采集数),env-step 计数不体现。**收敛对比改以 iteration 轴为准(fig6a)**,那张干净、无采集率混淆(TD3 同样收敛慢/低,是真结论)。删了 fig6c + 章节"样本效率"段,避免被 reviewer 攻击。
